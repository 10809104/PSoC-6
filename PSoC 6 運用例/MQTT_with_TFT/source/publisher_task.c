...
static void isr_button_press2(void *callback_arg, cyhal_gpio_event_t event);
...
static void publisher_init(void)
{
    ...
    printf("\nPress the user button to publish \"%s\"/\"%s\" on the topic '%s'...\n",
...
static void publisher_deinit(void)
{
    ...
    cyhal_gpio_register_callback(CYBSP_USER_BTN2, &cb_data2);
    cyhal_gpio_enable_event(CYBSP_USER_BTN2, CYHAL_GPIO_IRQ_FALL, USER_BTN_INTR_PRIORITY, false);
...
static void isr_button_press(void *callback_arg, cyhal_gpio_event_t event)
{
    ...
    else if (can_push == 1)
    {
    	...
    	xQueueSendFromISR(tft_page_q, &one, &xHigherPriorityTaskWoken);
//        if (TFT_handle_t != NULL) xTaskResumeFromISR(TFT_handle_t);
    }
