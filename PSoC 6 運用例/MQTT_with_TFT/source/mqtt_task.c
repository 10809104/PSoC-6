...
*******************************************************************************/
// uart handle
TaskHandle_t UART_handle_t = NULL;
...
void mqtt_client_task(void *pvParameters)
{
    ...
    if (pdPASS != xTaskCreate(uart_receive_task, "uart task", UART_TASK_STACK_SIZE, NULL, UART_TASK_PRIORITY, &UART_handle_t))
    {
    ...
    while (true)
    {
        ...
        if (pdTRUE == xQueueReceive(mqtt_task_q, &mqtt_status, portMAX_DELAY))
        {
            ...
            switch(mqtt_status)
            {
                ...
                {
                    /* Dent the publisher before initiating reconnection. */
