...
// tft trans
void call_tft(void *idk)
{
	(void) idk;
	tft_q = xQueueCreate(10, sizeof(int));
	while(1)
	{
		if (pdTRUE == xQueueReceive(tft_q, &tft_page_data, portMAX_DELAY))
		{
			printf("\n receive %d \r\n", tft_page_data);
			if(tft_page_data == 5)
			{
				xQueueSend(tft_page_q, &two, portMAX_DELAY);
			}
			else if(tft_page_data == 2)
			{
				xQueueSend(tft_page_q, &three, portMAX_DELAY);
			}
			else if(tft_page_data == 1)
			{
				xQueueSend(tft_page_q, &five, portMAX_DELAY);
			}
			else if(tft_page_data == 0)
			{
...
void subscriber_task(void *pvParameters)
{
    ...
    // create task for send msg to tft， idk whycan't use in here
    xTaskCreate(call_tft, "trans tft task", 512, NULL, 2, NULL);
...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info)
{
    ...
    if (car_license[6] == '*'  && tft_page_buf == 1)
	{
		if(received_msg_len >= 1)
		{
			can_push = 1;
			for (i=0; i<received_msg_len; i++)
			{
				car_license[i] = received_msg[i];
			}
		}
		xQueueSend(tft_q, &zero, portMAX_DELAY);
	}
    else if (car_license[7] == '5' && tft_page_buf != 2)
    {
    	if(received_msg_len >= 20)
    	{
        	for (i=0; i<19; i++)
        	{
        		time_in[i] = received_msg[i];
        	}
        	for (i=20; i<received_msg_len-2; i++)
        	{
        		money[i-20] = received_msg[i];
        	}
    	}
    	xQueueSend(tft_q, &five, portMAX_DELAY);
    }
    else if (car_license[7] == '2' && tft_page_buf != 3)
    {
    	if(received_msg_len >= 1)
    	{
        	for (i=0; i<received_msg_len; i++)
        	{
        		money[i] = received_msg[i];
        	}
    	}
    	xQueueSend(tft_q, &two, portMAX_DELAY);
    }
    else if (car_license[7] == '1' && tft_page_buf == 3)
    {
    	if(received_msg_len >= 1)
    	{
        	for (i=0; i<received_msg_len; i++)
        	{
        		balance[i] = received_msg[i];
        	}
    	}
    	xQueueSend(tft_q, &one, portMAX_DELAY);
    }
